...
def get_csvdata(dataframe):
    ...
    coordinates = Output, 4D array of plate identifier, coordinates and minimum magnitude
    """
    ...
    for i in range(len(coords)):
        ...
        # Convert coordinates to radians and add to array
        coords[i,1] = RA_arr[i] * np.pi/180
        coords[i,2] = Dec_arr[i] * np.pi/180
...
def main():
    ...
    with open("catlog_ukstu.lis", "r") as file:
        ...
        horizon_coords, horizon_errors = call_horizons('2022 PX1', JDs)
    ...
    #df = pd.read_csv("2mars.csv")
    coords = get_csvdata(df)
    mjds = get_MJdates(df)

    horizon_coords, horizon_errors = call_horizons('2024 YR4', mjds)
    proj, object_coords = projection(coords, horizon_coords, horizon_errors)
    ...
    #test coords from original catalogue are ok
    #print(coords[0:3])
    #print(dates[0:3])
    #print(JDs[0:3])
    #print(horizon_coords[0:3])
