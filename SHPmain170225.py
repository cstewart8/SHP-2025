...
def projection(plate_coords, horizon_coords):
    ...
    plate_coords = plate_coords[mask]
    horizon_coords = horizon_coords[mask]
    
    #if len(plate_coords) == 0:
        #return np.zeros((0, 2))

    # Initialise array to store projected coordinates
    projected_coords = np.zeros((len(plate_coords), 3))
    ...
    for i in range(len(plate_coords)):
        ...
        # Add plate number and projected coords to array
        projected_coords[i,0] = plate_coords[i,0]
        projected_coords[i,1] = xi
        projected_coords[i,2] = eta
    
    return projected_coords

def coordinate_comparison(projected_coords):
    """
    ...
    for i in range(len(projected_coords)):
        ...
        if -lim <= projected_coords[i,1] <= lim and -lim <= projected_coords[i,2] <= lim:
            ...
            print(f"Plate number: {int(projected_coords[i,0])} at x = {x_mm:.3f}mm, y = {y_mm:.3f}mm")
        else:
            count += 1
    ...
    if count == len(projected_coords):
        ...
        print(f"Object found on {hits} plates")
